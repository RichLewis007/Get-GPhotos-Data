...
- Logs all uncaught exceptions to the log file
...
- Shows the log file path for manual inspection
...
def install_exception_hook(
    ...
    """Log uncaught exceptions and present a user-friendly dialog.

    Args:
        error_dialog_factory: Optional callable(exc_type, exc, tb, log_path, parent)
    ...
    def excepthook(exc_type, exc, tb):  # type: ignore[no-untyped-def]
        ...
        with contextlib.suppress(Exception):
            ...
            if error_dialog_factory:
                dialog = error_dialog_factory(exc_type, exc, tb, log_path, parent)
                dialog.exec()
            else:
                # Lazy import only when needed (after QApplication exists)
                from ..dialogs.error_dialog import ErrorDialog

                dialog = ErrorDialog(exc_type, exc, tb, log_path, parent)
                dialog.exec()
